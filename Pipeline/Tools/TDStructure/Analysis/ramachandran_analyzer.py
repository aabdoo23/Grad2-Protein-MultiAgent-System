...
class RamachandranAnalyzer:
    ...
    def create_ramachandran_plot(self, angles_data, output_path=None):
        """
        Create a Ramachandran plot from phi/psi angle data.
        ...
        try:
            ...
            # Alpha helix region
            alpha_helix = patches.Rectangle((-90, -70), 60, 120, linewidth=2, 
                                          edgecolor='blue', facecolor='lightblue', 
                                          alpha=0.3, label='Alpha Helix')
            ax.add_patch(alpha_helix)
            
            # Beta sheet regions
            beta_sheet1 = patches.Rectangle((-180, 90), 90, 90, linewidth=2,
                                          edgecolor='green', facecolor='lightgreen',
                                          alpha=0.3, label='Beta Sheet')
            ax.add_patch(beta_sheet1)
            
            beta_sheet2 = patches.Rectangle((-180, -180), 90, 90, linewidth=2,
                                          edgecolor='green', facecolor='lightgreen',
                                          alpha=0.3)
            ax.add_patch(beta_sheet2)
            
            # Left-handed helix region
            left_helix = patches.Rectangle((30, -20), 60, 100, linewidth=2,
                                         edgecolor='orange', facecolor='moccasin',
                                         alpha=0.3, label='Left-handed Helix')
            ax.add_patch(left_helix)
    ...
    def generate_ramachandran_analysis(self, pdb_file, output_dir=None):
        ...
        try:
            ...
            plot_results = self.create_ramachandran_plot(angles_data, plot_path)
